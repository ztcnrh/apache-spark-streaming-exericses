...
spark = SparkSession.builder.appName("Customer-Join-Risk-Data").getOrCreate()
...
email_and_birth_year_df = email_and_birth_day_df.select('email', split(email_and_birth_day_df.birthDay, "-").getItem(0).alias("birthYear"))
...
joined_df = customer_risk_df.join(email_and_birth_year_df, expr("customer = email"))
