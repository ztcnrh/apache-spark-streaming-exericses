...
```
docker exec -it apache-spark-streaming-exericses-redis-1 redis-cli
```
...
```
docker exec -it apache-spark-streaming-exericses-kafka-1 kafka-console-consumer --bootstrap-server localhost:9092 --topic redis-server
```
...
```
docker logs apache-spark-streaming-exericses-spark-1 >& ../../spark/logs/spark-master.log

docker logs apache-spark-streaming-exericses-spark-worker-1-1 >& ../../spark/logs/spark-worker.log
```
...
```
docker logs -f apache-spark-streaming-exericses-stedi-1
```
...
- From a new terminal type: `submit-event-kafkajoin.sh` to submit to the cluster
